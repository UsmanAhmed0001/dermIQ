...
//
// WHAT THIS PAGE DOES:
// 1. Reads result + image from sessionStorage (set by the homepage)
// 2. Shows: uploaded image, top prediction with risk badge, full description,
//    clinical action advice, and confidence breakdown for all 7 classes
// 3. On mobile: everything stacks vertically for easy reading
//
// EDUCATOR NOTE:
// The confidence breakdown chart is the key differentiator vs competitors.
// Users can see WHY the model reached its conclusion — building trust.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  const handleShare = async () => {
    if (navigator.share) {
      await navigator.share({
        ...
        text: `My skin lesion was classified as: ${top.name} (${top.confidence}% confidence). Risk level: ${riskConf.label}. Please consult a dermatologist.`,
      })
    }
  }
  ...
  return (
    <main className="min-h-screen flex flex-col">
      ...
      <div className="max-w-2xl mx-auto w-full px-4 py-6 space-y-5">
        ...
        {/* ── IMAGE + TOP RESULT ────────────────────────────────────── */}
        <div className="glass rounded-2xl overflow-hidden" style={{ animationDelay: '0ms' }}>
          <div className="sm:flex">
            ...
            <div className="p-5 flex flex-col justify-center gap-3">
              <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium w-fit
                ${riskConf.bg} ${riskConf.text} border ${riskConf.border}`}>
        ...
        <div className={`rounded-2xl p-5 border ${riskConf.border} ${riskConf.bg}`}>
          <p className={`text-xs font-mono-custom uppercase tracking-widest mb-3 ${riskConf.text}`}>
            What to do
          </p>
            ...
            {result.allPredictions.map((pred, i) => (
              <div key={pred.lesionId}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className={`text-xs font-medium ${i === 0 ? 'text-white/90' : 'text-white/45'}`}>
                    {pred.name}
                  </span>
                  <span className={`text-xs font-mono-custom ${i === 0 ? 'gold-text' : 'text-white/35'}`}>
                    {pred.confidence}%
                  </span>
                </div>
